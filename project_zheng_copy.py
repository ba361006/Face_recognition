...
def load_data(): # 載入圖片時，把label與圖片的資料存到字典"imgs"裡，label起始值為1
    # , 'no_3':[], 'no_4':[], 'no_5':[], 'no_6':[], 'no_7':[], 'no_8':[], 'no_9':[], 'no_10':[]
    imgs = {'no_1': [], 'no_2': []}
    count = 0
...
class Vgg16:
    ...
    def __init__(self, vgg16_npy_path=None, restore_from=None):
        ...
        self.tfy = tf.placeholder(tf.float32, [None, 2])# 種類變多這裡要改
        ...
        self.out = tf.layers.dense(self.fc6, 2, name='out') # 種類變多這裡要改
...
def train():
    ...
    label_1, no1_x = label_split(no_1) # type(no1_x): list
    label_2, no2_x = label_split(no_2)
    print('label_1:',label_1)

    xs = np.concatenate(no1_x + no2_x, axis = 0)
    ys = np.concatenate((label_1, label_2), axis = 0)
...
def eval():
    ...
    vgg.predict(
        './For_transfer_learning/data/test_no_1/0350_01.jpg', labels = [0,2]) # labels[x,y], x:第幾種，從零開始, y總共幾種
