...
def train():
    ...
    for k in range(101): # 1001 0.00082637486
        count = 0
        for content in imgs.keys():
            train_start_one = time.time()
            random_idx= np.random.randint(10,99)

            train_loss = vgg.train(
                np.array(imgs[content][(random_idx-10):random_idx]).reshape(category,224,224,3),
...
# batch: 1 (?,224,224,3), 1e-8 訓練到10000次還是一直震盪，沒有收斂卡在0~7之間，正確率是垃圾37%左右而已

# batch: 10 (?,224,224,3), 1e-6 dict_batch的方法， 訓練一千次loss = 0.00094769493
# 正確率 27% 63% 83%